...
LvDevice::LvDevice(LvWindow &window) : window{window} {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
...
void LvDevice::createSurface() { window.create_window_surface(instance, &surface_); }
...
void LvDevice::setupDebugMessenger() {
...
void LvDevice::hasGflwRequiredInstanceExtensions() {
...
QueueFamilyIndices LvDevice::findQueueFamilies(VkPhysicalDevice device) {
...
SwapChainSupportDetails LvDevice::querySwapChainSupport(VkPhysicalDevice device) {
...
uint32_t LvDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void LvDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
